...
type CLI_Eval struct {
	FUNC Func[CLI_Eval_Input]
}
type CLI_Eval_Input struct {
	Opt_Format string
...
func resolve_CLI_Eval_Input(input *CLI_Eval_Input, restArgs []string) error {
	*input = CLI_Eval_Input{
	...
	for idx, arg := range restArgs {
		...
		case "-pretty", "-p":
			...
			if err := parseValue(&input.Opt_Pretty, lit); err != nil {
				return fmt.Errorf("value %q is not assignable to option %q", lit, optName)
			}
...
func Run(cli CLI, args []string) error {
	...
	case "eval":
		funcMethod := cli.Eval.FUNC
		if funcMethod == nil {
			return fmt.Errorf("%q is unsupported: cli.Eval.FUNC not assigned", "eval")
		}
		var input CLI_Eval_Input
		err := resolve_CLI_Eval_Input(&input, restArgs)
		return funcMethod(subcommandPath, input, err)
...
func resolveSubcommand(args []string) (subcommandPath []string, restArgs []string) {
	...
	subcommandSet := map[string]bool{
		"":     true,
		"eval": true, "validate": true, "version": true,
	}
